...
|---|---|---|---|
| V1 | **Red–green ramps fail for ~8% of male viewers** (deuteranopia). The classic speed ramp (green→red) is the worst case. | major | offer a **colorblind-safe option** (`--cmap cb`); the DEFAULT stays the intuitive meaning-first green->red ramp practitioners read instantly (color *sensitivity*/convention first, per user) (cividis / viridis for continuous; blue–orange diverging for signed) with red–green as an opt-in; keep **redundant encoding** (width AND color carry different variables, so color-blind readers still get flow from width). |
