...
## Fix 4 — Static figures (`renderers/gmns_figures.py`): connector pollution + hardcoded ranges  (F-010)

**Problem (what the user sees).** Ziyi reviewed the static PNG catalog (`datasets/*/figures/`) and
found four things that didn't look right: `by_free_speed` highlighted almost nothing (34/2950 links on
Chicago); `by_link_type`'s legend showed uninterpretable raw codes "1"/"2"/"3"; `dist_capacity` had an
isolated bar out at ~50,000 with a wide empty gap before it; `dist_free_speed`'s x-axis ran out to 500
while every visible bar stopped around 190, with no unit on the axis. He asked whether these were
Chicago-specific — **they weren't**; auditing all 6 datasets showed the same code paths fail (silently,
differently) on every one.

**Before / after.**

![Static figures before/after](evidence/F010_static_figures_before_after.png)

**Root cause — one pattern, four symptoms, all from `renderers/gmns_figures.py`:**

1. **Centroid connectors carry synthetic placeholder values baked into `link.csv`** (same fact as F-009):
   on Chicago, all 774 connectors have `capacity` fixed at 49,500 veh/h and `free_speed` fixed at exactly
   96.54 mph. `fig_distribution()` and `fig_by_categorical()` never excluded them, so "real road" charts
   were statistically polluted by virtual links. West Jordan (114/378 connectors) has the same issue;
   Sioux Falls and the toy nets happen to have zero connectors so weren't affected by this part.
2. **`by_free_speed`/`by_length` used a hardcoded absolute range** (`[10, 40]` mph, `[0.1, 2]` mi) that
   doesn't generalize: Chicago matched 34/2950 (1%), Sioux Falls/toys matched 0 (all links have a flat
   60 mph placeholder speed), **West Jordan matched 362/378 (96%)** — never a meaningful highlighted
   subset, regardless of dataset.
3. **`length` field priority bug**, Chicago only: the reader picked the bare `length` column (in
   **meters**, 98–61,714) over the explicitly mile-labeled `vdf_length_mi` column — the opposite
   priority from `ai-gen/gui4gmns.py`'s own dashboard generator. `by_length`'s `[0.1, 2]`-mile filter
   then matched 0 links against meter-scale numbers and silently produced no file.
4. **The export summary lied.** `export_all()` counted a job as successful whenever its function ran
   without raising — even if the function internally decided there was nothing to draw and returned
   without saving anything. "15 native gmns figures exported" was printed while only 8 files existed.

**The changes:**
- `read()`: added `is_conn` (same `capacity*lanes >= 40000` sentinel `ai-gen/gui4gmns.py` uses) and fixed
  `len` to prefer `vdf_length_mi` over `length`.
- `fig_distribution()`, `fig_by_categorical()`, `fig_by_range()`: all exclude `is_conn` links now.
- `fig_by_range()`: `by_free_speed`/`by_length` switched from a hardcoded absolute range to a **percentile
  band** (`pct=1/3`, "slowest/shortest third") computed from that dataset's own real-link data — adapts
  automatically to any speed/length scale or unit, sidestepping the whole units question. `by_lanes` kept
  an absolute `[2,4]` range (lane count is a genuine unit-agnostic physical category).
- `fig_distribution()`: view is now clipped to the 1st–99th percentile when outliers would otherwise
  flatten the real distribution, with an explicit "N link(s) outside […] not shown (view clipped, data
  unchanged)" note — the underlying values are never altered or dropped, only the display range. Axis
  labels now carry units ("free-flow speed (mph)").
- Every `fig_*` function now returns its saved path or `None`; `export_all()` reports an honest
  `"N figure(s) written: …"` / `"N skipped (no applicable data): …"` summary instead of a single
  misleading count.

**Effect, Chicago:** capacity histogram's isolated 49,500 spike is gone (2,176 real links shown, 22
off-scale noted); free-speed axis now spans 37–163 mph (unit-labeled) instead of an empty stretch to
500; `by_free_speed` highlights 726 links (33%, computed "slowest third") instead of 34 (1%); `by_length`
now produces a file at all (was silently empty); `by_link_type`'s legend shows only the 2 real road
classes, not 3 with a connector-inflated one. **Verified on all 6 datasets** — West Jordan's
previously-inverted `by_free_speed` (96% match) now shows a real 33% band; Sioux Falls/toys, which have
zero speed variation, now report a clear one-line skip reason instead of silently doing nothing.

**Not touched.** No CSV values were edited. The four genuinely anomalous Chicago links with a 200–500
mph nominal free-flow speed are a real data-quality issue in the source network — left alone, only kept
from dominating the chart's axis.
